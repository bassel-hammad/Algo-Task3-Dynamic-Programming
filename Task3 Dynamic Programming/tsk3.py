...
class MainWindow(QMainWindow):
    ...
    def Generate(self):
         # Perform seam carving (reduce width by 50 pixels)
            target_width = self.original_image.shape[1] - 50
            resized_image = self.seam_carve(self.original_image, target_width)

            # Save and display the resized image
            output_file_path = "resized_image.jpg"
            cv2.imwrite(output_file_path, resized_image)
            self.FrameImg(self.outputFrame, output_file_path)  # Display the resized image
    ...
    def seam_carve(self,image, target_width):
